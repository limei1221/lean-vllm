...
Names mirror vLLM's under an `inferweave:` prefix, so one dashboard reads both
...
class Metrics:
    ...
    def __init__(self):
        ...
        self.running = Gauge("inferweave:num_requests_running", "Requests in the running set.")
        self.waiting = Gauge("inferweave:num_requests_waiting", "Requests in the waiting queue.")
        self.kv_usage = Gauge("inferweave:gpu_cache_usage_perc", "Fraction of KV blocks in use.")

        self.requests_received = Counter("inferweave:num_requests_received_total", "Requests admitted.")
        self.requests_rejected = Counter("inferweave:num_requests_rejected_total", "Requests refused by admission control.")
        self.requests_aborted = Counter("inferweave:num_requests_aborted_total", "Requests cancelled by their client.")
        self.requests_finished = Counter(
            "inferweave:request_success_total", "Requests that ran to a finish.", label="finish_reason"
        )
        self.preemptions = Counter("inferweave:num_preemptions_total", "Sequences preempted to free blocks.")

        self.prompt_tokens = Counter("inferweave:prompt_tokens_total", "Prompt tokens of finished requests.")
        self.generation_tokens = Counter("inferweave:generation_tokens_total", "Tokens generated by finished requests.")
        self.prefill_tokens = Counter("inferweave:prefill_tokens_total", "Prompt tokens run through the model.")
        self.decode_tokens = Counter("inferweave:decode_tokens_total", "Decode rows run through the model.")

        self.prefix_cache_queries = Counter("inferweave:prefix_cache_queries_total", "Blocks looked up at admission.")
        self.prefix_cache_hits = Counter("inferweave:prefix_cache_hits_total", "Blocks the prefix cache supplied.")

        self.steps = Counter("inferweave:num_steps_total", "Forward passes.")
        self.graph_steps = Counter("inferweave:num_graph_steps_total", "Forward passes replayed from a CUDA graph.")
        self.model_busy = Counter("inferweave:model_busy_seconds_total", "Wall seconds spent inside a step.")

        self.ttft = Histogram("inferweave:time_to_first_token_seconds", "Arrival to first token.", LATENCY_BUCKETS)
        self.tpot = Histogram("inferweave:time_per_output_token_seconds", "Mean seconds per token after the first.", TPOT_BUCKETS)
        self.queue_time = Histogram("inferweave:request_queue_time_seconds", "Arrival to first schedule.", LATENCY_BUCKETS)
        self.e2e = Histogram("inferweave:e2e_request_latency_seconds", "Arrival to finish.", LATENCY_BUCKETS)
        self.request_prompt_tokens = Histogram("inferweave:request_prompt_tokens", "Prompt length.", TOKEN_BUCKETS)
        self.request_generation_tokens = Histogram("inferweave:request_generation_tokens", "Completion length.", TOKEN_BUCKETS)
        self.step_duration = Histogram("inferweave:step_duration_seconds", "Wall time of one step.", STEP_BUCKETS)
        self.iteration_tokens = Histogram("inferweave:iteration_tokens_total", "Tokens in one step's batch.", BATCH_BUCKETS)
